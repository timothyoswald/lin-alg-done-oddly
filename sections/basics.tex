...
\begin{example}
    ...
    \begin{enumerate}
        ...
        \item The set of all polynomials of degree less than or equal to $5$ over $\mathbb{R}$
    \end{enumerate}
\end{example}
...
\end{remark}
A more formal definition of a vector space is as follows:
\begin{definition}
    Let $(E,+)$ be an abelian group and let $(\mathbb{K}, +,\cdot)$ be a field. We say that $E$ is a vector space over $\mathbb{K}$ if there exists a map $\mathbb{K}\times E\to E$ via $(\alpha,x)\mapsto\alpha\cdot x$ called scalar multiplication satisfying four axioms:
    \begin{enumerate}
        \item For all $x\in E$, $1_\mathbb{K}\cdot x=x$
        \item For all $\alpha,\beta\in\mathbb{K}$,$x\in E$, $(\alpha+\beta)\cdot x=\alpha x+\beta x$
        \item For all $\alpha,\beta\in\mathbb{K}$, $(\alpha\beta)\cdot x=\alpha\cdot(\beta\cdot x)$.
        \item For all $\alpha\in\mathbb{K}$ and all $x,y\in E$, $\alpha\cdot(x+y)=\alpha\cdot x+\alpha\cdot y$.
    \end{enumerate}
\end{definition}
If you do not know what a group is, you can just ignore this definition and move on. But I think that even if you don't know what that is, you can believe that this is equivalent to the previous definition based on the properties.

\begin{example}
    Let $V$ be a vector space over a field $\mathbb{F}$ and let $A$ be a non-empty subset. We define $E^A$ to be the set of all functions from $A$ to $E$. We define vector addition to be $f,g\in E^A$ then $(f+g)(x)=f(x)+g(x)$ and for $\lambda\in\mathbb{F}$, we have that $\lambda f(x)=\lambda f(x)$
\end{example}
...
\end{proof}
\begin{remark}
    If a problem says a vector space $V$ over a field $\mathbb{F}$, you can make no assumptions about the vector space and field outside of the axioms.
\end{remark}
...
\end{remark}
In general, to prove that $U\subseteq V$ is a subspace, you need to check that $\vec{0}\in U$, and $U$ is closed under operations. You should convince yourself that, as mentioned in the previous remark, the other axioms follow from the fact that $U\subseteq V$.
\begin{remark}
...
\begin{definition}
    Let $V$ be a vector space with $W_1,W_2,\dots, W_n$ being subspaces of $V$. We define the sum of these to be:
    $$W_1+W_2+\dots+W_n=\{x_1+x_2+\dots+x_n:x_i\in W_i\}$$
\end{definition}

\begin{definition}
    Let $V,W$ be vector spaces. We say that $V+W$ are in direct sum if for all $\vec{u}\in V+W$, we can write $\vec{u}=\vec{v}+\vec{w}$ uniquely for $\vec{v}\in V$ and $\vec{w}\in W$. This is denoted $V\oplus W$.
\end{definition}
...
\begin{exercise}
    Consider the vector space of $\R$ over $\R$. Show that the only subspaces are $\{0\}$ and $\R$.
\end{exercise}

\begin{exercise}
    Show that the intersection of arbitrarily many subspaces is a subspace.
\end{exercise}

\begin{exercise}
    Show that the union of two subspaces is not necessarily a subspace.
\end{exercise}
...
\begin{definition}
    Let $V$ be a vector space. A basis of $V$ is a list $L\subseteq V$ which satisfies the following two properties:
    \begin{enumerate}
        \item $L$ is linearly independent
        \item $span(L)=V$
    \end{enumerate}
...
\end{corollary}
\begin{proof}
    From Theorem $1.6.5$, we showed that linearly independent sets are at most the size of any spanning set. Since a basis is a spanning set and it has size $n$, all linearly independent lists must be at most size $n$. So any list larger than size $n$ must be linearly dependent.
\end{proof}
There exists a similar statement to Corollary $1.6.6$ regarding span which is left as an exercise. You should realize that these statements are immensely powerful. You need not know a single thing about any of the vectors than the list other than there are "too many" or "too few" vectors in a list and you immediately know facts about span and linear independence.
\begin{theorem}
...
\begin{proof}
    ...
    You can repeat this process until the $span(L)=V$. We've shown that at every step the new list is still linearly independent. Now we must show that it stops when $k=n$ and that the $span(L)=V$ at that point. If the process did not stop at $k=n$, then we would add another vector and the list would still be linearly independent. However, this is impossible since we would then have a list of $n+1$ linearly independent vectors. The only way for the process to stop is if there are no more vectors in $V$ that are not in $span(L)$. Thus, the process stops when $k=n$ and $span(L)=V$.
\end{proof}
...
\end{definition}
\begin{remark}
    The coordinate vector essentially describes the vector $\vec{v}$ as a vector of its coefficients with respect to a basis $B$. Here is becomes important that we defined a basis as a list and not a set. This means that it is ordered so the coordinate vector is well-defined.
\end{remark}
\begin{remark}
...
\section{Exercises 2}
\begin{exercise}
    Prove that (1) and (3) imply each other in Theorem 1.5.4.
\end{exercise}
\begin{proof}
